...
class Lead(Base):
    ...
    x_date = Column(String(50), nullable=True)
...
class LeadQuote(Base):
    """Cached AgencyZoom lead quotes (read-only)."""
...
class LeadFile(Base):
    """Cached AgencyZoom lead file references (read-only)."""
...
class LeadNote(Base):
    """Cached AgencyZoom lead notes (read-only)."""
