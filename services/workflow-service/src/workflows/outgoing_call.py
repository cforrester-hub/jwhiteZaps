...
async def process_single_call(call: dict, mark_as_processed_callback=None) -> dict:
    ...
    # Empty/invalid numbers should be skipped - we can't match them in AgencyZoom
    phone_digits = "".join(c for c in to_number if c.isdigit())
    if len(phone_digits) < 7:
        logger.info(f"Call {call_id} has invalid destination number '{to_number}', skipping")
